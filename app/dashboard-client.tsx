...
export default function DashboardClient({ displayName, role, organizationName }: { displayName: string; role: "manager" | "employee" | "auditor"; organizationName: string }) {
    ...
    <div className="shell">
      <aside className={menu ? "sidebar open" : "sidebar"}>
        ...
        <div className="sidebar-foot">
          ...
          <div className="profile"><Avatar text={initials(displayName)} color="navy" /><div><strong>{displayName}</strong><small>{roleLabel(role)} · {organizationName}</small></div><span>•••</span></div>
        </div>
      </aside>
      ...
      <main>
        <header className="topbar">
          ...
          <div><small>{organizationName} / {page}</small><h1>{page === "Today" ? `Good morning, ${firstName(displayName)}` : page}</h1></div>
...
function roleLabel(role: "manager" | "employee" | "auditor") { return role === "manager" ? "Manager" : role === "employee" ? "Employee" : "Auditor"; }
